  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-lg border px-2.5 py-1.5 text-xs font-semibold transition-all duration-200 ${
        devPro
          ? "border-[color:var(--neon-green)]/50 bg-[color:var(--neon-green)]/10 text-[color:var(--neon-green)] shadow-[var(--glow-green)]"
          : "border-border bg-surface/60 text-muted-foreground hover:text-foreground"
      }`}
    >
      <FlaskConical className="h-3.5 w-3.5" />
      <span className="hidden sm:inline">Dev Mode</span>
      <span className={`h-4 w-7 shrink-0 rounded-full p-0.5 transition-colors duration-200 ${devPro ? "bg-[color:var(--neon-green)]/40" : "bg-muted"}`}>
        <span
