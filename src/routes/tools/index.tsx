...
function ToolsIndex() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-10 md:px-8 md:py-14">
      <h1 className="text-4xl font-semibold tracking-tight md:text-5xl">{t("tools.title")}</h1>
      <p className="mt-3 max-w-2xl text-sm text-muted-foreground md:text-base">{t("tools.subtitle")}</p>

      <h2 className="mt-12 text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--primary)]">{t("tools.free")}</h2>
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
        {FREE_TOOLS.map((tool) => <ToolCard key={tool.slug} tool={tool} unlocked />)}
      </div>

      <h2 className="mt-14 text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--primary)]">{t("tools.pro")}</h2>
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
      ...
      <div className="glass mt-14 flex flex-wrap items-center justify-between gap-4 p-6">
        ...
        <Link to="/billing" className="inline-flex items-center gap-2 rounded-xl bg-[color:var(--primary)] px-5 py-2.5 text-sm font-semibold text-[color:var(--primary-foreground)] hover:opacity-90">
...
function ToolCard({ tool, unlocked }: { tool: Tool; unlocked: boolean }) {
  ...
  return (
    <Link
      ...
      className={`glass group flex flex-col p-5 transition hover:translate-y-[-2px] hover:shadow-[var(--shadow-soft)] ${tool.span ?? ""}`}
    >
      <div className="flex items-start justify-between gap-3">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-surface-2 text-[color:var(--primary)]">
          <tool.icon className="h-5 w-5" />
        </span>
        {!unlocked && (
          <span className="inline-flex items-center gap-1 rounded-full bg-[color:var(--primary)]/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-[color:var(--primary)]">
