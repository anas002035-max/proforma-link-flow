    ...
    <div className="p-6 md:p-10">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="glass p-6">
          <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold"><QrCode className="h-4 w-4 text-[color:var(--primary)]" /> Quick generator</h2>
          ...
          <input
            ...
            placeholder="https://…"
            className="mt-1 w-full rounded-lg border border-input bg-surface px-3 py-2 text-sm outline-none focus:border-[color:var(--primary)] focus:shadow-[var(--shadow-soft)]"
          />
        ...
        <div className="glass p-6 lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/links" className="inline-flex items-center gap-1 text-xs text-[color:var(--primary)] hover:underline">
          ...
          {links.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
              {links.map((l) => (
                <Link key={l.id} to="/links/$id" params={{ id: l.id }} className="glass flex flex-col items-center gap-2 p-4 transition hover:shadow-[var(--shadow-soft)]">
                  <QRPreview value={`${origin}/api/public/r/${l.slug}`} fg="#f8fafc" bg="#FFFFFF" size={140} />
                  <div className="w-full truncate text-center text-xs font-semibold">{l.title || l.slug}</div>
                  <div className="w-full truncate text-center text-[10px] text-[color:var(--primary)]">/{l.slug}</div>
                </Link>
              ))}
            </div>
          )}
